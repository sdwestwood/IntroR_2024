...
Sometimes we may want to alter certain values within a column or vector. Conditional statements are used when you want to create an output value that is conditioned on a logical expression being `TRUE`. This is similar to the function `filter()`, which retains columns based on a logical expression evaluating to `TRUE`. Here, we are showing you how to use them in combination with `mutate()` by either modifying the values within one column, or by adding a new column.
...
* The left hand side determines the logical expression that is being applied 
* The right hand side provides the replacement value when the logical expression is `TRUE`
* The `.default` argument sets the value for all remaining rows
...
```{r}
fake_data <- mutate(fake_data, 
                    Institution_case_when = case_when(
                      Institution == 1 ~ "UoG",
                      Institution == 3 ~ "GCU",
                      Institution == 4 ~ "UWS",
                      .default = "Strathclyde"
...
`case_match()` functions works essentially as a particular use case of `case_when` that automatically applies the match operator `%in%` for the logical expression (rather than >, ==, <= etc).  

The use of `case_match()` is slightly different to `case_when()`:

* The first argument is the column that the match operator is being applied to
* Each subsequent argument also takes the form of a formula
* The left hand side determines which values match this case 
* The right hand side provides the replacement value
* The `.default` argument sets the value for all remaining rows


```{r, eval=FALSE}
mutate(data, new_column_name = case_match(column, old_value ~ new_value))
```

Let's repeat the previous example with `case_match` instead of `case_when`:

```{r}
fake_data <- mutate(fake_data, 
                    Institution_case_match = case_match(
                      Institution,
                      1 ~ "UoG",
                      3 ~ "GCU",
                      4 ~ "UWS",
...
Add a column `grade_new` into `fake_data` using a combination of `mutate()` and either `case_when()` or `case_match()`, that recodes grade points to primary bands. It is up to you to choose the function that you think makes the most sense here. If you have extra time, try both ways!
...
# using case_match
fake_data <- mutate(fake_data, 
                    grade_new = case_match(Grade_points, 
                                           18:22 ~ 'A',
                                           15:17 ~ 'B',
                                           12:14 ~ 'C',
                                           9:11  ~ 'D',
...
# using case_when
fake_data <- mutate(fake_data, 
                    grade_new2 = case_when(Grade_points %in% 18:22 ~ 'A',
...
# we could also use .default = 0 at the end, but bear in mind this will change any value not yet covered by the previous conditional statements to "H" - this might not always be correct!
```
`r unhide()`
