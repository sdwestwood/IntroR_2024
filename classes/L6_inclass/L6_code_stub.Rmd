...
fake_data <- mutate(
  fake_data, 
  Sex_ifelse = ifelse(Sex==1, 'Male', 'Female')
...
```{r}
fake_data <- mutate(fake_data,
                    Pass_Fail = ifelse(Grade_points>9,'Pass','Fail'))
glimpse(fake_data)
```
...
```{r}
fake_data <- mutate(fake_data, 
                    Grade = ifelse(Grade_points %in% 18:22, "A",
...
When using this to add a new column, this takes the following form: 
...
fake_data <- mutate(
  fake_data,
  Institution_case_when = case_when(Institution == 1 ~ 'UoG',
                                    Institution %in% c(2,99) ~ 'Strathclyde',
                                    Institution == 3 ~ 'GCU',
                                    TRUE ~ 'UWS')
)
head(fake_data)
...
```{r}
fake_data_no_dup <- select(fake_data,-PP_ID) %>%
  distinct()
head(fake_data_no_dup)
```
...
Let's play around with `fake_data` whilst recapping a little bit of what we did in the past 5 weeks.
...
```{r}
fake_data <- select(fake_data,
                    Participant_ID = PP_ID,
                    Sex = Sex_char, 
                    Institution = Institution_char,
                    Age, 
                    Grade_points,
                    Grade,
                    Pass_Fail)
