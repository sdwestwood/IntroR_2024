...
4. Keep only the information about the number of people that came into and left the Scotland (we do not want to know about the net value)
...
```{r}
ggplot(data = traffic_scot, aes(x = Sex, y = Value))
...
Create a column/bar plot to show the data use `geom_col()`. What does the data look like?
...
Q: Considering what we know from the data description and the different variables in it, do we need another variable to make the plot more informative? If yes, which one?
...
Imagine you want to see the difference in the number of people coming into and moving out of Scotland from both the UK and Overseas throughout the years, but you do not want to know the net flow of people, their age or their sex. 

Q: What Variables would you use?

**Write your answer here**
A: DateCode, Value, `Migration Source`, `Migration Type`
...
```{r}
# I would actually argue the coloured shapes are not helpful
ggplot(traffic_scot2, aes(DateCode, Value, 
                          shape = `Migration Source`
                          )) + 
  geom_point() + 
  geom_line(aes(colour = `Migration Type`))
...
female_migration <- migration_scot %>%
  ...
  select(Value, `Migration Type`)
  

# make box plots
ggplot(female_migration, aes(`Migration Type`, Value, fill = `Migration Type`)) + 
  geom_violin() +
  geom_jitter(width = .1)
...
Bonus Challenge! Represent the data in `migration_scot` in any way you like, but make two versions: one pretty version and one ugly version. Use the ggplot cheatsheet and/or the internet to help you figure out some cool customisation. 
