---
title: "Tidy Data 1: Principles and Pivots"
author: "SW"
date: "22/11/2023"
...
Undo the messiness by reshaping the data in `sun_fail` into a tidier format. Store the tidy data in your `Global Environment` as `sun_retidy`. Your final dataset should have the columns `year`, `season`, and `sun` in *exactly* that order.
...
Now, that we have learnt about the `mutate()` function, we can attempt to get all of these row names into a new column. All we have to do is combining a function called `rownames()` with `mutate()`. We also could use some more of these handy pipes `%>%` you learnt about.
...
```{r}
df_mtcars <- select(df_mtcars, Car_type, mpg:carb)
df_mtcars
```
