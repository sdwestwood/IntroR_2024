...
* Add a new column to `Name_Data` that is called `prcnt` and is position *to the left* of `prop` that gives the percentage each name accounts for of total names that year. *Hint: `prop` (proportion) is like a percentage represented out of 1 instead of out of 100! 
...
* Rename the column `Success` to be called `WinRate`, and keep only the following columns in this specific order:
...
If done correct `wick6pipe` should look identical to `wick6f`
...
AND THEN
use the arrange() function to sort first by `Reach` in *desceding* order, then by `Tallness` in *ascending* order. 
...
The resulting table should be stored in `Example_6`.
```
...
Store the result in a variable called `Example_7`.
```
